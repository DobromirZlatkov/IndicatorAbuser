...
//+------------------------------------------------------------------+
extern double DISTANCE_TO_SL_TO_OPEN_AVERAGING = 10;
...
void OnTick()
  {
      ...
      if(numberOfOpenOrdersForCurrentDayByMagic(MAGIC) == 0 && numberOfPastOrdersForCurrentDayByMagic(MAGIC) == 0)
        ...
        if(numberOfOpenOrdersForCurrentDayByMagic(AVERAGING_MAGIC) == 0 && numberOfPastOrdersForCurrentDayByMagic(AVERAGING_MAGIC) == 0 && OrdersTotal() > 0)
        {
            double orderStopLoss = todayOrderStopLossValue(MAGIC);
            double orderTp = todayOrderTakeProfitValue(MAGIC);
            
            if(isOrderSell(MAGIC))
            {
               if(orderStopLoss - Bid <= DISTANCE_TO_SL_TO_OPEN_AVERAGING)
                 {
                    OrderSend(Symbol(), OP_SELL, LOT_SIZE, Bid, SLEEPAGE, orderStopLoss, orderTp, "Strategy Averaging", AVERAGING_MAGIC);
                 }
            }
            if(isOrderBuy(MAGIC))
            {
                if(Ask - orderStopLoss <= DISTANCE_TO_SL_TO_OPEN_AVERAGING)
                 {
                    OrderSend(Symbol(), OP_BUY, LOT_SIZE, Ask, SLEEPAGE, orderStopLoss, orderTp, "Strategy Averaging", AVERAGING_MAGIC);
                 }
            }
        }
...
double todayOrderStopLossValue(int magic)
{
   double stopLoss = 0;
   int totalOrders = OrdersTotal();  
...
double todayOrderTakeProfitValue(int magic)
{
   double takeProfit = 0;
   int totalOrders = OrdersTotal();  
